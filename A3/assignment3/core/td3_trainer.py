...
class TD3Trainer:
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            noise = (
                torch.tensor(np.random.normal(0.0, self.policy_noise))
            ...
            critics = self.critic_target(next_state, next_action)
            target_Q = reward + self.discount * torch.mul(not_done, torch.minimum(critics[0], critics[1]))
            # pass
        ...
        critic_loss = 1 / batch_size * torch.sum((current_Q1 - target_Q)**2 + (current_Q2 - target_Q)**2)
        # pass
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            actor_loss = -1 / batch_size * torch.sum(self.critic.Q1(state, self.actor(state)))            
            # pass
