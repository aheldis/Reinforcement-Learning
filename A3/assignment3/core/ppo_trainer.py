...
class PPOTrainer:
    ...
    def compute_action(self, obs, deterministic=False):
        ...
        if self.discrete:  # Please use categorical distribution.
            ...
            # pass
            actions = dist.probs.argmax(dim=1, keepdim=True) if deterministic else dist.sample().view(-1, 1)
    ...
    def compute_loss(self, sample):
        ...
        clamp = torch.clamp(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param) * adv_targ
        policy_loss = -torch.minimum(ratio * adv_targ, clamp).mean()
        # pass

        policy_loss_mean = policy_loss.mean()

        # TODO: Implement value loss
        value_loss = (return_batch - values)**2
        # pass
